...
import { Copy, ExternalLink, Heart, HeartIcon, RefreshCw, Share2 } from "lucide-react";
...
export function ResultCard({ result, nameType, onCopy, onRefresh, index, isRefreshing }: ResultCardProps) {
  ...
  const handleShare = async () => {
    const statusEmoji = verification.status === 'available' ? '✅' : 
                       verification.status === 'similar' ? '⚠️' : '❌';
    
    const shareText = `🎵 Check out this ${nameType} name I generated with NameJam:\n\n` +
                      `${nameType === 'band' ? '🎸' : '🎤'} "${name}"\n` +
                      `${statusEmoji} Status: ${getStatusText(verification.status)}\n\n` +
