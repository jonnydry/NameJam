...
- **API Timeout Improvement**: Increased external API timeout from 3 seconds to 15 seconds to prevent premature timeouts with Spotify, Last.fm, and ConceptNet APIs
- **Applied Indexes**: 
  - generatedNames: userId, type, createdAt, (userId + createdAt), (type + userId + createdAt)
  - errorLogs: userId, createdAt, (userId + createdAt)
- **Result**: Faster database queries, smaller bundle size, better API reliability, and modern API usage
...
- January 28, 2025. Expanded name generation results from 4 to 8 total names per request: maintained 50/50 split between AI-assisted and traditional generations (4 AI + 4 Datamuse); provides more variety and entertainment value for users; leverages existing rich API context from Spotify, Last.fm, ConceptNet, and Datamuse to support 8 high-quality results; updated both backend default count and frontend request count for consistency; ensures all 8 results meet core mission of entertainment, delight, and thought-provoking creativity
- January 28, 2025. Enhanced band bio generation with diverse journalistic styles: added 8 new journalistic writing styles (Rolling Stone Pretentious, TMZ Gossip, NPR Tiny Desk, Pitchfork Hipster, Behind The Music Documentary, Small Town Newspaper, Gen Z Music Blog, Academic Thesis) alongside original edgy style; implemented detailed JSON prompt structures for XAI API with comprehensive specifications for task, style requirements, story elements, and humor guidelines; 50/50 chance of using new journalistic styles vs original style for maximum variety; each style has custom system prompt and specific voice instructions; all prompts prioritize being wickedly funny, entertaining, and witty with absurd fictional elements
