...

{
  ...
  private async generateWithOptimizedXAI(
    ...
    try {
      secureLog.info(`🚀 grok-4-fast optimized generation: ${config.temperature} temp, ${config.max_tokens} tokens`);
      ...
      if (!content) {
        throw new Error('No content generated by grok-4-fast');
      }
      ...
      // If insufficient results, return what we have (better than complex retry logic)
      if (uniqueNames.length < count) {
        secureLog.debug(`Generated ${uniqueNames.length}/${count} names - sufficient for quality over quantity`);
      }
      
      return uniqueNames.slice(0, count);

    } catch (error) {
      secureLog.error('Optimized XAI generation error:', error);
      throw error;
    }
  }
