...
export class LyricStarterService {
  ...
  constructor() {
    ...
    if (process.env.XAI_API_KEY) {
      try {
        ...
        secureLog.error(
          "Failed to initialize OpenAI client for lyric generation:",
          error,
        );
        this.openai = null;
      }
  ...
  async generateLyricStarter(
    genre?: string,
  ): Promise<{ lyric: string; model: string; songSection?: string }> {
    ...
    try {
      ...
      const currentSection =
        songSections[Math.floor(Math.random() * songSections.length)];
      ...
      const selectedRhyme =
        rhymeSchemes[Math.floor(Math.random() * rhymeSchemes.length)];
...
- Professional songwriting quality`,
      ...
      if (generatedResponse) {
        try {
          ...
          if (lyric && lyric.length > 5 && lyric.length < 200) {
            // Clean the lyric
            const cleanLyric = lyric.replace(/^["'""']|["'""']$/g, "").trim();

            secureLog.info(
              `Successfully generated lyric: "${cleanLyric}" using model: ${model} with Datamuse context`,
            );
            return {
              lyric: cleanLyric,
              model: model,
  ...
  private generateFallbackLyric(genre?: string): {
    lyric: string;
    model: string;
    songSection: string;
  } {
    ...
    const sections = Object.keys(fallbackLyrics) as Array<
      keyof typeof fallbackLyrics
    >;
    ...
    const selectedLyric =
      sectionLyrics[Math.floor(Math.random() * sectionLyrics.length)];
  ...
  private async getComprehensiveAPIContext(genre?: string): Promise<{
    ...
    try {
      ...
      apiPromises.push(
        this.fetchDatamuseContext(genre || "contemporary", context),
      );
      ...
      secureLog.debug(
        `🎵 Comprehensive API context for ${genre || "contemporary"}: ${totalWords.length} total vocabulary items from all APIs`,
      );
    } catch (error) {
  ...
  private async fetchDatamuseContext(
    genre: string,
    context: any,
  ): Promise<void> {
    try {
      ...
      const selectedRhyme =
        rhymeSeeds[Math.floor(Math.random() * rhymeSeeds.length)];
      ...
      secureLog.debug(
        `✅ Datamuse context: ${context.datamuse.genreWords.length + context.datamuse.emotionalWords.length + context.datamuse.rhymeWords.length} words fetched`,
      );
    } catch (error) {
  ...
  private async fetchSpotifyContext(
    genre: string,
    context: any,
  ): Promise<void> {
    try {
      if (await this.spotifyService.isAvailable()) {
        ...
        secureLog.debug(
          `✅ Spotify context: ${context.spotify.genreArtists.length} artists, ${context.spotify.moodTracks.length} tracks`,
        );
      }
    } catch (error) {
  ...
  private async fetchLastFmContext(genre: string, context: any): Promise<void> {
    try {
      ...
      if (genreVocabulary) {
        ...
        context.lastfm.topArtists = genreVocabulary.descriptiveWords.slice(
          0,
          8,
        );
        context.lastfm.relatedGenres = genreVocabulary.relatedGenres.slice(
          0,
          5,
        );

        secureLog.debug(
          `✅ Last.fm context: ${context.lastfm.topArtists.length} descriptive words, ${context.lastfm.relatedGenres.length} related genres`,
        );
      }
    } catch (error) {
  ...
  private async fetchConceptNetContext(
    genre: string,
    context: any,
  ): Promise<void> {
    try {
      ...
      const emotionalConcepts =
        await conceptNetService.getEmotionalAssociations(genre);
      context.conceptnet.emotionalConcepts = emotionalConcepts.slice(0, 8);

      // Get cultural associations
      const culturalAssociations =
        await conceptNetService.getCulturalConnections(genre);
      context.conceptnet.culturalAssociations = culturalAssociations.slice(
        0,
        6,
      );

      secureLog.debug(
        `✅ ConceptNet context: ${context.conceptnet.genreConcepts.length} concepts, ${context.conceptnet.emotionalConcepts.length} emotions`,
      );
    } catch (error) {
  ...
  private async getDatamuseContext(genre?: string): Promise<{
    ...
    try {
      ...
      context.contrastWords = this.getContrastFromEmotion(
        emotionalSeeds[0],
        genre,
      );
      ...
      secureLog.debug(
        `🎵 Enhanced Datamuse context for ${genre || "contemporary"}: ${allWords.size} unique words`,
      );
    } catch (error) {
  ...
  private getGenreSeedWords(genre?: string): string[] {
    ...
    return (
      genreSeeds[genre || ""] || ["music", "song", "melody", "rhythm", "sound"]
    );
  }
  ...
  private getTempoSeeds(genre?: string): string[] {
    ...
    return (
      tempoMap[genre || "pop"] || ["steady", "flowing", "moving", "rhythmic"]
    );
  }
  ...
  private async fetchPoetryContext(
    ...
    try {
      ...
      secureLog.debug(
        `🎭 Poetry context for ${genre || "general"}: ${poetryContext.poeticPhrases.length} phrases, ${poetryContext.vocabulary.length} words`,
      );
    } catch (error) {
